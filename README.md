...
- 目标消失后异常重现保护
- 异常帧定位、候选确认及撤销
- 传播后并列检查首帧掩码；单正点目标会提示局部分割风险和补点方法
- 可直接在首帧结果上选择目标、补正负点、撤销最后一点并在同一任务中重新传播
- 多目标掩码重叠提示
- 本地历史任务预览和再次导出
- 历史任务占用预览、精确确认、可恢复清理和 Web 回收区恢复
- 新建任务遇到同名编号时拒绝覆盖已有目录
- 抽帧失败或中断的任务会保留状态；可在历史任务中查看，并按确认流程移入可恢复回收区
- 视频默认限制为 200 MiB、300 帧；超限时提示裁剪，不继续抽取整段视频
...
300 帧是当前短视频试用的资源保护值，可在 `configs/default.json` 中调整；提高上限前需重新验证磁盘、内存和推理耗时。
...
- 同一条 `crossing` 序列还完成了两名行人与一辆货车的三目标联合传播；三者各保存完整 52 帧掩码，逐对象 J&F 为 0.9511、0.9658、0.9739。这是单序列结果，不是 DAVIS 全集成绩。
- `bmx-trees` 自行车道序列完成了自行车与骑行者的 80 帧联合传播。自行车首帧 IoU=0.6427，未达到预设 0.7 门槛，保留为失败案例；骑行者首帧通过，中间 78 帧 J&F=0.8623。轮圈误覆盖和树干遮挡是下一步修正的重点，不据此声称补齐城市道路评估。
- 候选确认现在只恢复本次重新出现到下一次重新出现之间的掩码，不会一次放行后续所有候选；该边界已通过工程回归，尚无真实二次重现视频验证。
- 最近一次自动化测试结果为 `91 passed`（2026-09-25）；新增评价器输入检查，避免错误对象 ID、非二值掩码等生成误导性成绩。
...
- 已完成道路车辆真值评估，以及 `crossing` 道路行人和货车的三目标联合真值评估；尚无带真值的道路非机动车评估。
- 历史任务目前只能预览和再次导出，尚不能恢复成可继续补点的 SAM2 会话。
- 清理只会把任务移入项目回收区，不释放空间；Web 可恢复，永久过期清理尚未实现。
